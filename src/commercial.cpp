...
Commercial::Commercial(std::shared_ptr<std::vector<std::vector<char>>> regionState, std::shared_ptr<int> workers, std::shared_ptr<int> goods) : regionState(regionState), workers(workers), goods(goods)  {
    ...
    for (int i = 0; i < this->regionState->size(); i++) {
        for (int j = 0; j < this->regionState->at(i).size(); j++) {
            if (this->regionState->at(i).at(j) == 'C') {
...
bool Commercial::applyRules(bool changed) {
    
    //Stop region from growing if enough workers dont exist
    if(*this->workers < 1) return changed;
